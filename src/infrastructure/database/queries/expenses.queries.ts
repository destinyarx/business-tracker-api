...
import { CreateExpenseDto } from '../../../features/expenses/dto/create-expense.dto';
import { UpdateExpenseDto } from '../../../features/expenses/dto/update-expense.dto';
import { GetExpensePaginateDto } from '../../../features/expenses/dto/get-expense-paginate-dto';
import {
...
export async function getExpensesPaginated(
	query: GetExpensePaginateDto,
	userId: string,
) {
	const { limit, offset, searchKey, category, paymentMethod, timePeriod } =
		query;
	let hasNext = false;

	const range = timePeriod
		? getDateRangeFromPeriod(timePeriod as TimePeriod)
		: undefined;
...
export async function updateExpense(
	...
	return await db
		...
		.where(and(eq(expenses.id, id), eq(expenses.createdBy, userId)))
