...
import { ItemDto } from '../../../features/orders/dto/update-order-status-dto';
...
async function updateProductQuantity(
	...
	const stockById = new Map(
		currentProducts.map((item) => [item.id, item.stock]),
	);
	...
	const caseExpr = sql`CASE ${products.id} ${sql.join(
		orderItems.map(
			(i) => sql`WHEN ${i.product.id} THEN ${updatedStock[i.product.id]}`,
		),
		sql` `,
